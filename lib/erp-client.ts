...
export class ERPClient {
  ...
  constructor() {
    ...
    if (!this.apiKey || !this.apiSecret) {
      console.warn(
        "[v15] Warning: ERP_API_KEY or ERP_API_SECRET not set. API calls may fail."
      );
    }
  }
  ...
  private async fetch<T>(
    ...
    const url = endpoint.startsWith("/api")
      ? `${this.erpUrl}${endpoint}`
      : `${this.erpUrl}/api${endpoint}`;
  ...
  private async getSalesInvoicesFallback(company: string) {
    const params = new URLSearchParams({
      fields: JSON.stringify(["name", "customer", "outstanding_amount"]),
  ...
  private async getPurchaseInvoicesFallback(company: string) {
    const params = new URLSearchParams({
      fields: JSON.stringify(["name", "supplier", "outstanding_amount"]),
